...
function adicionar_fase(){
    ...
    document.getElementById('perguntas').innerHTML += `<p>&nbsp&nbsp&nbsp&nbspFase ${quantidade_fase}: Clique para adicionar uma nova pergunta a essa fase (minimo de 4 e maximo de 8)
...
function adicionar_pergunta(fase){
    if(fases[fase] === 8){
        alert('Limite maximo alcançado na fase ' + fase)
    } else {
        fases[fase] += 1
...
function deletar_pergunta(fase){
    if(fases[fase] === 2){
        alert('Minimo de fases alcançado na fase ' + fase)
    } else {
